    ...
    for i, j in ti.ndrange(res + 1, res):
        if i > 0 and i < res:
            if phi[i - 1, j] < 0 or phi[i, j] < 0:
                if solid_phi[i - 1, j] >= 0 and solid_phi[i, j] >= 0 and ball_phi(i - 1, j) > 0 and ball_phi(i, j) > 0:
                    u[i, j] -= p[i, j] - p[i - 1, j]
    for i, j in ti.ndrange(res, res + 1):
        if j > 0 and j < res:
            if phi[i, j - 1] < 0 or phi[i, j] < 0:
                if solid_phi[i, j - 1] >= 0 and solid_phi[i, j] >= 0 and ball_phi(i, j - 1) > 0 and ball_phi(i, j) > 0:
                    v[i, j] -= p[i, j] - p[i, j - 1]
